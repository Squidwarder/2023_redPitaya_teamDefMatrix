...
#* helper function to detect function timeout
...
thefile = "received_signal_long.txt"                    # Stores received signal inside this file

plaintext_list = []                                     # Stores the list of plaintext words

#! keeps track if the function timed out (takes too long to process signal)
#start = time.time() 
#signal.signal(signal.SIGBREAK, handler) 

def receive():
    #* keeps track of true number of buffers
    true_buff_num = 0
    ...
    for i in range(reps):
        ...
        curr_buff_morse = decode_text.decode_from_list(buff[i, :])
        #print(len(curr_buff_morse))

        curr_plaintext = decode_gui.morse_decode_plain(curr_buff_morse)
        plaintext_list.append(curr_plaintext)
    ...
    for j in range(true_buff_num):
        ...
        if j == 0:
            ...
            with open(thefile, "a") as text_file:
                for em in buff[j]:
                    text_file.writelines(str(em))
                    text_file.write("\n")

    return " ".join(plaintext_list[1:len(plaintext_list) - 1])

def run():
    try: 
        return receive() 
    except: 
        return "Function manually stopped after not processing (or receiving) signal"
