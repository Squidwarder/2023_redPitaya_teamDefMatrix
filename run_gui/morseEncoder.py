...
def encode(text):
    ...
    for words in text:
        encoded = ""
        #! ensures that words are in uppercase to eliminate key errors
...
def add_signal(morse):
    ...
    #! arbitrarily add K ending signal 
    return "111010111010111"+ "0000000" + morse + "0000000" + "111010111"

def pulse(morse):
    m_pulse =""
    for i in morse:
        if i == "1":
            m_pulse = m_pulse+("10"*7)
        else:
            m_pulse = m_pulse+("00"*7)
    return m_pulse
        
'''
plaintext = input("Enter a sentence(word) to be encrypted [only capital letters]: ")
ptext_list = plaintext.split(" ")
print(pulse(add_signal(encode(ptext_list))))
'''
